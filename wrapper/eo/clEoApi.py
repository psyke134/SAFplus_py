...
class eClEOServiceInstallOrderT(clUtils.CEnum):
...
CL_EO_USER_CLIENT_ID_START = eClEoClientIdT.CL_EO_CLOVIS_RESERVED_CLIENTID_END.value

CL_EO_SERVER_COOKIE_BIT_SIZE = 8

def CL_EO_SERVER_COOKIE_BASE(cId):
    _cId = cId.value
    return _cId << CL_EO_SERVER_COOKIE_BIT_SIZE
...
class eClEoServerIdT(clUtils.CEnum):
...
class eClEoCrashReasonT(clUtils.CEnum):
