...
class eClLogSeverityT(clUtils.CEnum):
    ...
    CL_LOG_SEV_ALERT = enum.auto(),
    CL_LOG_SEV_CRITICAL = enum.auto(),
    CL_LOG_SEV_ERROR = enum.auto(),
    CL_LOG_SEV_WARNING = enum.auto(),
    CL_LOG_SEV_NOTICE = enum.auto(),
    CL_LOG_SEV_INFO = enum.auto(),
    CL_LOG_SEV_DEBUG = enum.auto(),
    CL_LOG_SEV_DEBUG1   =      CL_LOG_SEV_DEBUG,
    CL_LOG_SEV_DEBUG2 = enum.auto(),
    CL_LOG_SEV_DEBUG3 = enum.auto(),
    CL_LOG_SEV_DEBUG4 = enum.auto(),
    CL_LOG_SEV_DEBUG5 = enum.auto(),
    CL_LOG_SEV_TRACE  =    CL_LOG_SEV_DEBUG5,
    CL_LOG_SEV_DEBUG6 = enum.auto(),
    CL_LOG_SEV_DEBUG7 = enum.auto(),
    CL_LOG_SEV_DEBUG8 = enum.auto(),
    CL_LOG_SEV_DEBUG9 = enum.auto(),
...
def clLogStreamOpen(hLog, streamName, streamScope, pStreamAttr, streamOpenFlags, timeout, phStream):
    ...
    return clLib.libmw_so.clLogStreamOpen(
        ...
        streamName,
        streamScope,
        ctypes.byref(pStreamAttr),
        streamOpenFlags,
        timeout,
        ctypes.byref(phStream)
    )
...
def clLogWriteAsyncWithContextHeader(hStream, severity, pArea, pContext, serviceId, msgId, *va_args):
    ...
    return clLib.libmw_so.clLogWriteAsyncWithContextHeader(hStream, severity, ctypes.byref(pArea), ctypes.byref(pArea), serviceId, msgId, *cVaArgs)
...
def clLogHandlerRegister(hLog, streamName, streamScope, nodeName, handlerFlags, phStream):
    ...
    return type:
        ClRcT
    """
    return clLib.libmw_so.clLogHandlerRegister(hLog, streamName, streamScope, nodeName, handlerFlags, ctypes.byref(phStream))
...
def clLogFileMetaDataGet(hFileHdlr, pStreamAttr, pNumStreams, ppLogStreams):
    ...
    return clLib.libmw_so.clLogFileMetaDataGet(
        hFileHdlr,
        ctypes.byref(pStreamAttr),
        ctypes.byref(pNumStreams),
        ctypes.byref(ppLogStreams)
    )
