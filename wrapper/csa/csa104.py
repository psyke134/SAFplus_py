...
def clCompAppAMFCSISet(invocation, compName, haState, csiDescriptor):
    ...
    if haState == saAmf.eSaAmfHAStateT.SA_AMF_HA_ACTIVE.value:
        ...
        # for the CSI.
        #
        clprintf(clLogApi.eClLogSeverityT.CL_LOG_SEV_INFO, "csa104: ACTIVE state requested; activating message queue receiver service")
        ...
        clprintf(clLogApi.eClLogSeverityT.CL_LOG_SEV_INFO, "csa104: Standby state requested")
        ...
        clprintf(clLogApi.eClLogSeverityT.CL_LOG_SEV_INFO, "csa104: Acknowledging new state quiesced")
        ...
        clprintf(clLogApi.eClLogSeverityT.CL_LOG_SEV_INFO, "csa104: Signaling completion of QUIESCING")
...
def msgInitialize():
    ...
    rc = saMsg.saMsgInitialize(
        ctypes.byref(msgLibraryHandle),
        ctypes.byref(MsgCallbacks),
        ctypes.byref(version)
    )
    if rc != saAis.eSaAisErrorT.SA_AIS_OK.value:
        clprintf(
...
def msgOpen(queuename, bytesPerPriority):
    rc = saAis.eSaAisErrorT.SA_AIS_OK
    saQueueName = saAis.SaNameT(queuename)
    ...
    rc = saMsg.saMsgQueueOpen(
        msgLibraryHandle,
        ctypes.byref(saQueueName),
        ctypes.byref(CreationAttributes),
        OpenFlags,
        timeout,
        ctypes.byref(msgQueueHandle)
    )

    if rc != saAis.eSaAisErrorT.SA_AIS_OK.value:
        clprintf(
            clLogApi.eClLogSeverityT.CL_LOG_SEV_ERROR,
            "Msg QueueOpen failed [0x%X]\n\r",
            rc
        )
...
def msgSend(queuename, buffer, length):
    ...
    message.type = 0
    message.version.releaseCode = ord('B')
    ...
    if rc != saAis.eSaAisErrorT.SA_AIS_OK.value:
        clprintf(
            clLogApi.eClLogSeverityT.CL_LOG_SEV_ERROR,
            "Msg saMsgMessageSend to queue [%s] failed [0x%X]",
            saQueueName.value,
            rc
        )
...
def msgReceiverLoop():
    ...
    while True:
        ...
        rc = saMsg.saMsgMessageGet(
            msgQueueHandle,
            ctypes.byref(message),
            ctypes.byref(SendTime),
            ctypes.byref(SenderId),
            saAis.SA_TIME_MAX
        )
        ...
        if rc != saAis.eSaAisErrorT.SA_AIS_OK.value:
            clprintf(
                clLogApi.eClLogSeverityT.CL_LOG_SEV_ERROR,
                "Msg saMsgMessageGet failed [0x%X]\n\r",
                rc
            )
            break
