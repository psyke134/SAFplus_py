...
def clCreatePipe(fds, numMsgs, msgSize):
    ...
    arg types:
        ...
        ClUint32T msgSize
    """
    clLib.libmw_so.clCreatePipe(fds, numMsgs, msgSize)
...
class eClDispatchFlagsT(clUtils.Enum):
    CL_DISPATCH_ONE         = 1
    CL_DISPATCH_ALL         = 2
...
class eClEoWaterMarkFlagT(clUtils.Enum):
    CL_WM_LOW_LIMIT = 0
    CL_WM_HIGH_LIMIT = 1

ClWaterMarkIdT = saAis.SaInt32T
class eClWaterMarkIdT(clUtils.Enum):
    CL_WM_LOW = 0
    CL_WM_HIGH = 1
    CL_WM_MED = 2
    CL_WM_SENDQ = 3
